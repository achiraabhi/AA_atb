...
class Measurement:
    ...
    def __init__(self, b: bytes):
        ...
        self._data['display_unit'] = self._UNITS.get(self._data['mode'], {}).get(self._data['range'])
        if self._data['display_unit'] is None:
            # unknown mode/range combination - don't crash, just report it
            log.warning('no unit for mode=%s range=%s', self._data['mode'], self._data['range'])
            self._data['display_unit'] = '?'
        ...
        if self._data['unit'][0] in self._EXPONENTS and not self._data['overload']:
            self._data['decimal'] = self._data['decimal'].scaleb(self._EXPONENTS[self.unit[0]])
...
class UT61EPLUS:
    ...
    # Some units (e.g. UT61B+ hardware revisions) ship a WCH HID-UART bridge
    # ("WCH UART TO KB-MS", vendor-defined HID page 0xFFA0) instead of the
    # Silicon Labs CP2110. It speaks the same UT61 protocol but needs no
    # CP2110 init and expects an HID report-id 0x00 prefix on writes.
    WCH_VID = 0x1a86
    WCH_PID = 0xE429
    ...
    def __init__(self, path=None, is_wch=None):
        ...
        try:
            ...
            # fall back to the WCH bridge (e.g. UT61B+)
            self.dev.open(self.WCH_VID, self.WCH_PID)
            self._wch = True
            log.debug('WCH device is open (no CP2110 init needed)')

    def _write(self, b: bytes):
        buf = []
        if self._wch:
            buf.append(0x00)  # HID report id
        buf.append(len(b))
        buf += b
        self.dev.write(buf)
